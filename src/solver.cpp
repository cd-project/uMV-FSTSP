...
Result Solver::mvdSolver(int n_thread, int e) {
    ...
    for (int k:V) {
        t[k] = model.addVar(0, GRB_INFINITY, 0.0, GRB_CONTINUOUS,"t_" + std::to_string(k));
    }
    model.addConstr(t[O] == 0);
    ...
    for (int k = 0; k < n; k++) {
        ...
        model.addConstr(t[k+1] >= t[k] + sum, "C_time_between" + std::to_string(k+1) + "_" + std::to_string(k));
    }
    for (int k:V) {
        for (int k_p:V) {
            if (k < k_p) {
            for (int h:C) {
                ...
                model.addConstr(t[k_p] - t[k] >= rhs);
            }
        }}
    }
    GRBVar max = model.addVar(0, GRB_INFINITY, 0.0, GRB_CONTINUOUS, "obj");

    GRBLinExpr objective;
    for (int k:V) {
        model.addConstr(max >= t[k], "C_obj>=t_" + std::to_string(k));
    }
    objective += max;
    model.setObjective(objective, GRB_MINIMIZE);
    model.update();
